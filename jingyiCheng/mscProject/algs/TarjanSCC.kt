...
(G: Digraph) {
    ...
    private val id: IntArray = IntArray(G.V)                // id[v] = id of strong component containing v
    private val low: IntArray = IntArray(G.V)               // low[v] = low number of v
    private var pre: Int = 0                 // preorder number counter
    var count: Int = 0               // number of strongly-connected components
        private set
    private val stack: nnStack<Int> = nnStack()
    ...
    private fun validateVertex(v: Int) {
        if (v < 0 || v >= marked.size) throw IllegalArgumentException("vertex $v is not between 0 and ${marked.size - 1}")
    }
