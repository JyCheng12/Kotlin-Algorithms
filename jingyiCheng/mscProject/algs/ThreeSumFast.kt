 ...
 * Dependencies: StdOut.kt In.kt Stopwatch.kt
...
object ThreeSumFast {
    ...
    private fun containsDuplicates(a: IntArray): Boolean {
    ...
    fun printAll(a: IntArray) {
        val n = a.size
        a.sort()
        if (containsDuplicates(a)) throw IllegalArgumentException("array contains duplicate integers")
        for (i in 0 until n)
            for (j in i + 1 until n) {
                val k = a.binarySearch(-(a[i] + a[j]))
